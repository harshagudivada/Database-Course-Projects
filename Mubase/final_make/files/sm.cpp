...
int StorageManager::getFirstBlockId(const char *dbName, 
	...
	BufferManager *bm = BufferManager::getInstance(dbName);
	 ...
	 cout<<"Given object is not found\n";
	 bm->unpinPage(0,false); //added	                       
}
	...
	for(int i=0;i<nofObjects;i++){
		...
		if( strcmp(objName,objectName) == 0 ){
			ObjectFirstBlockId = readInt(diskFirstBlock, 
										2*SIZE_INT + i*OBJ_SIZE +
										OBJNAME_SIZE);
			int nextBlockId = ObjectFirstBlockId;
			//Assuming Object to be in Slotted Page organization
			while( nextBlockId != -1 ) {
				char *objectFirstBlock = bm->pinPage(nextBlockId);
				int id = readShort(objectFirstBlock, BLOCK_SIZE-2);
				bm->unpinPage(nextBlockId,false);
				freeBlock(dbName, nextBlockId);
				nextBlockId = id;
			}
